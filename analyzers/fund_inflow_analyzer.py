"""
analyzers/fund_inflow_analyzer.py
[Phase 2 / T3] 시총 대비 집중 자금유입 트리거 분석기 (v3.2 신규)
...
데이터 소스: pykrx 마감 확정치 (일별 OHLCV + 시가총액)
...
[ARCHITECTURE 의존성]
fund_inflow_analyzer → reports/closing_report (호출)
"""
...
def analyze(date_str: str, top_n: int = None) -> list[dict]:
    ...
    logger.info(f"[fund_inflow] T3 시총 대비 자금유입 분석 시작 — {date_str}")
    ...
    for market in ["KOSPI", "KOSDAQ"]:
        try:
            ...
            logger.warning(f"[fund_inflow] {market} 조회 실패: {e}")
            continue
    ...
    logger.info(
        f"[fund_inflow] T3 완료 — 시총 {config.FUND_INFLOW_CAP_MIN/1e8:.0f}억원↑ "
