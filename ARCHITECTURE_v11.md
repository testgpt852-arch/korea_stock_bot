...
2. **개편·수정 작업 순서 (반드시 준수)**
   ...
   - 코드 작업 진행
   - 완료 후 변경된 규칙이 있으면 **RULES.md도 업데이트**
   - changelog에 한 줄 추가
...
│   ├── dart_collector.py
│   ├── event_calendar_collector.py  ← EVENT_CALENDAR_ENABLED=false 기본
...
06:00  지정학 뉴스 수집 (GEOPOLITICS_ENABLED=true 시만)
       ① geopolitics_collector.collect() → raw_news
          Reuters / Bloomberg / 기재부 / 방사청 / Google News RSS
          소스 실패 → 비치명적 (빈 리스트 반환)
...
[{"title": str, "summary": str, "url": str,
  "source": str,     # "reuters" | "bloomberg" | "moef" | "dapa" | "google_news"
