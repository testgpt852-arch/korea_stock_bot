"""
analyzers/volume_flat.py
[Phase 2 / T6] 횡보 거래량 급증 트리거 분석기 (v3.2 신규)
...
[ARCHITECTURE 의존성]
volume_flat → reports/closing_report (호출)
volume_flat → utils/watchlist_state (선택적 저장 — 내일 워치리스트 보강용)
"""
...
def analyze(date_str: str, top_n: int = None) -> list[dict]:
    ...
    logger.info(f"[volume_flat] T6 횡보 거래량 급증 분석 시작 — {date_str}")
    ...
    for market in ["KOSPI", "KOSDAQ"]:
        try:
            ...
            if df_prev.empty:
                logger.warning(f"[volume_flat] {market} 전일 데이터 없음 — 전일 데이터 조회 실패")
                continue
            ...
            logger.warning(f"[volume_flat] {market} 조회 실패: {e}")
            continue
