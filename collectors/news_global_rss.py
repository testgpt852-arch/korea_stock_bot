"""
collectors/news_global_rss.py  [v12.0]
글로벌 RSS + 정부 RSS + Google News + GDELT 수집 전담 (키 없이 무료)

[분리 경위]
v12.0: news_collector.py를 3개로 분리 (Naver / NewsAPI / GlobalRSS)
  - 이 파일: AP·FT RSS + korea.kr 정부 RSS 15개 + Google News + GDELT
  - API 키 불필요, 완전 무료 동작
  기존 geopolitics_collector.py 내용 흡수 + 장중 수집 스케줄 제거 (dead code)

[원본]
geopolitics_collector.py (v14.0) 내용 통합 — 파일 삭제됨
...
def collect(max_per_source: int = 20) -> list[dict]:
    ...
    if not config.GEOPOLITICS_ENABLED:
        logger.info("[news_global_rss] GEOPOLITICS_ENABLED=false — 수집 건너뜀")
        return []
    ...
    for source in _RSS_SOURCES:
        try:
            ...
            logger.info(f"[news_global_rss] {source['name']}: {len(items)}건 수집")
        except Exception as e:
            # rule #90: 비치명적 — 소스 실패해도 계속 진행
            logger.warning(f"[news_global_rss] {source['name']} 실패 (무시): {e}")
    ...
    try:
        ...
        logger.info(f"[news_global_rss] GDELT: {len(gdelt_items)}건 수집")
    except Exception as e:
        logger.warning(f"[news_global_rss] GDELT 실패 (무시): {e}")
    ...
    if config.NEWSAPI_ENABLED:
        try:
            ...
            logger.info(f"[news_global_rss] NewsAPI.org: {len(newsapi_items)}건 수집")
        except Exception as e:
            logger.warning(f"[news_global_rss] NewsAPI.org 실패 (무시): {e}")
    ...
    logger.info(f"[news_global_rss] 총 {len(unique)}건 수집 완료 (중복 제거 후)")
    return unique
...
def _fetch_rss(
    ...
    try:
        ...
        logger.warning(f"[news_global_rss] {name} HTTP 요청 실패: {e}")
        return []
    ...
    if not feed.entries:
        if feed.bozo:
            logger.warning(f"[news_global_rss] {name} RSS 파싱 오류 + entries 없음: {feed.bozo_exception}")
        else:
            logger.debug(f"[news_global_rss] {name} entries 없음")
        return []

    if feed.bozo:
        logger.debug(f"[news_global_rss] {name} bozo=True이나 entries={len(feed.entries)}건 수집 진행")
...
def _fetch_google_news_rss(max_per_query: int = 5) -> list[dict]:
    ...
    for query in _GOOGLE_NEWS_QUERIES:
        try:
            ...
            logger.warning(f"[news_global_rss] Google News '{query}' 실패 (무시): {e}")
    ...
    for query in _NEWSAPI_GEO_QUERIES:
        try:
            ...
            if data.get("status") != "ok":
                logger.warning(f"[news_global_rss/newsapi] API 오류 '{query}': {data.get('message')}")
                continue
            ...
            logger.warning(f"[news_global_rss/newsapi] 쿼리 실패 '{query}' (무시): {e}")
...
def _fetch_gdelt_geopolitics(max_per_query: int = 5) -> list[dict]:
    ...
    for query in _GDELT_GEO_QUERIES:
        try:
            ...
            logger.warning(f"[news_global_rss/gdelt] 쿼리 실패 '{query}' (무시): {e}")
